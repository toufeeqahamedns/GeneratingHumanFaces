...
class Discriminator(th.nn.Module):
    ...
    def __init__(self, depth=7, feature_size=512,
        ...
        self.final_block = DisFinalBlock(
            self.feature_size * 2, use_eql=self.use_eql)
...
class ConditionalDiscriminator(th.nn.Module):
    """ Conditional Discriminator of the GAN """
    ...
    def __init__(self, depth=7, feature_size=512, embedding_size=4096, compressed_latent_size=128,
        ...
        from torch.nn import ModuleList
        from GAN.CustomLayers import DisGeneralConvBlock, \
            ConDisFinalBlock, _equalized_conv2d
        from torch.nn import Conv2d
    ...
    def __init__(self, depth=7, latent_size=512,
        ...
        if device == th.device("cuda"):
            ...
            self.dis = Discriminator(
                depth, latent_size, use_eql=True).to(device)
...
class ConditionalGAN:
    ...
    def __init__(self, depth=7, latent_size=512, ca_hidden_size=4096, ca_out_size=256, loss_fn="relativistic-hinge",
        ...
        if device == th.device("cuda"):
            self.gen = DataParallel(self.gen)
            self.dis = ConditionalDiscriminator(depth, latent_size, ca_hidden_size, ca_out_size,
                                                use_eql=use_eql, gpu_parallelize=True).to(device)
        else:
    ...
    def train(self, data, gen_optim, dis_optim, ca_optim, text_encoder, encoder_optim, normalize_latents=True,
        ...
        # Causing error will work on this later
        ...
        for epoch in range(start, num_epochs + 1):
            ...
            if epoch % checkpoint_factor == 0 or epoch == 1 or epoch == num_epochs:
                ...
                ca_save_file = os.path.join(
                    save_dir, "CA_" + str(epoch) + ".pth")
                gen_save_file = os.path.join(
                    save_dir, "GAN_GEN_" + str(epoch) + ".pth")
                dis_save_file = os.path.join(
                    save_dir, "GAN_DIS_" + str(epoch) + ".pth")
                ca_optim_save_file = os.path.join(
                    save_dir, "CA_OPTIM_" + str(epoch) + ".pth")
                gen_optim_save_file = os.path.join(save_dir,
                                                   "GAN_GEN_OPTIM_" + str(epoch) + ".pth")
                dis_optim_save_file = os.path.join(save_dir,
                                                   "GAN_DIS_OPTIM_" + str(epoch) + ".pth")
